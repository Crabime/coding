...
org.activemq.Amq = function() {
  ...
  var clientId = null;
	...
	var sendJmsMessage = function(destination, message, type, headers) {
		...
		// Add message to outbound cn.crabime.queue
		if (batchInProgress) {
	...
	return {
		...
		// Send a JMS message to a destination (eg cn.crabime.topic://MY.TOPIC).  Message
		// should be xml or encoded xml content.
		sendMessage : function(destination, message) {
			sendJmsMessage(destination, message, 'send');
		},

		// Listen on a channel or cn.crabime.topic.
		...
		//
		// Example: addListener( 'handler', 'cn.crabime.topic://test-cn.crabime.topic', function(msg) { return msg; }, { selector: "property-name='property-value'" } )
		...
		// remove Listener from channel or cn.crabime.topic.
